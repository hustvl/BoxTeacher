...
class FCOS(nn.Module):
    ...
    def forward_test(self, images, features, gt_instances=None, top_module=None, post_nms_topk=100):
        features = [features[f] for f in self.in_features]
        locations = self.compute_locations(features)
        logits_pred, reg_pred, ctrness_pred, top_feats, bbox_towers = self.fcos_head(
            features, top_module, self.yield_proposal or self.yield_box_feats
        )
        self.fcos_outputs.eval()
        # awful code here, much awful
        post_nms_topk_ = self.fcos_outputs.post_nms_topk_test
        self.fcos_outputs.post_nms_topk_test = post_nms_topk
        results = self.fcos_outputs.predict_proposals(
            logits_pred, reg_pred, ctrness_pred,
            locations, images.image_sizes, top_feats
        )
        self.fcos_outputs.post_nms_topk_test = post_nms_topk_
        extras = {}
        if self.yield_box_feats:
            extras["box_feats"] = {
                f: b for f, b in zip(self.in_features, bbox_towers)
